...
<?php include '../../includes/header.php'; ?>
    <div class="container">
        <h1>Project naam Zoekresultaten</h1>
...
if (isset($_GET["markeer"])) {
    ...
    if ($emailResult->num_rows > 0) {
        ...
        if (mail($ontvanger, $onderwerp, $bericht, $headers)) {
            ...
            $query = "SELECT * FROM bestellingen WHERE project_naam = '$project'";
            $result = $conn->query($query);

            if ($result->num_rows > 0) {
                echo "<h2>Bestellingen gevonden voor project: $project</h2>";
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($row['besteld'] == 0) {
                        echo "<td>
                                  <a class='btn btn-primary' href='?project=$project&markeer=" . $row['id'] . "' onclick='return confirm(\"Weet u zeker dat u deze bestelling wilt markeren als besteld?\")'>Besteld</a>
                                  <a class='btn btn-danger' href='./afkeur.php/?id=" . $row['id'] . "'>Afkeuren</a>
                              </td>";
                    } else {
                        echo "<td>
                    <a class='btn btn-danger' href='?project=$project&verwijder=" . $row['id'] . "' onclick='return confirm(\"Weet u zeker dat u deze bestelling wilt verwijderen?\")'>Verwijderen</a> <!-- Nieuwe kolom -->
                </td>";
                    }
                ...
                echo "<p>Geen bestellingen gevonden voor project: $project</p>";
            }
        ...
        if (isset($_GET["verwijder"])) {
            ...
            echo "<p>Ongeldig project naam.</p>";
        }
        ?>

        <a class="btn btn-primary" href="../project">Terug naar de project zoekfunctie</a>
    </div>
</body>
</html>